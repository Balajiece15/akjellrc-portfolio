...
export class SpeedRunStorage {
  ...
  static async getAllSpeedRuns(): Promise<SpeedRun[]> {
    try {
      ...
      const runs = await client.get(this.SPEED_RUNS_KEY)
  ...
  static async addSpeedRun(run: Omit<SpeedRun, 'id'>): Promise<SpeedRun> {
    try {
      ...
      runs.push(newRun)
      await client.set(this.SPEED_RUNS_KEY, JSON.stringify(runs))
      return newRun
    } catch (error) {
  ...
  static async deleteSpeedRun(id: number): Promise<boolean> {
    try {
      ...
      await client.set(this.SPEED_RUNS_KEY, JSON.stringify(filteredRuns))
      return true
    } catch (error) {
...
export class BuildLogStorage {
  ...
  static async getAllBuildEntries(): Promise<BuildEntry[]> {
    try {
      const client = await getRedisClient()
      const entries = await client.get(this.BUILD_ENTRIES_KEY)
  ...
  static async addBuildEntry(entry: Omit<BuildEntry, 'id'>): Promise<BuildEntry> {
    try {
      ...
      await client.set(this.BUILD_ENTRIES_KEY, JSON.stringify(entries))
      return newEntry
    } catch (error) {
  ...
  static async deleteBuildEntry(id: number): Promise<boolean> {
    try {
      ...
      await client.set(this.BUILD_ENTRIES_KEY, JSON.stringify(filteredEntries))
      return true
    } catch (error) {
...
export class DatabaseManager {
  ...
  static async getProjectKeys(): Promise<string[]> {
    try {
      const client = await getRedisClient()
      const keys = await client.keys(`${PROJECT_NAMESPACE}:*`)
      return keys
    } catch (error) {
  ...
  static async clearProjectData(): Promise<boolean> {
    try {
      const client = await getRedisClient()
      await Promise.all([
        client.del(SpeedRunStorage['SPEED_RUNS_KEY']),
        client.del(BuildLogStorage['BUILD_ENTRIES_KEY'])
      ])
      return true
    } catch (error) {
